 ...
 * 根据配置过滤并生成对应的menu列表
 */
export const filterMenuItms = (list: RouteItem[], prefixPath = ''): Array<ReturnType<typeof touchItem>> =>
  list
    .filter((item) => item.hide !== true)
    .map((item) => {
      const path = `${prefixPath}/${item.path.replace(/^\/+/, '')}`
      const parent = path.replace(/\/+$/, '')
      const children = item.children ? filterMenuItms(item.children, parent) : []

      return touchItem(item.label, path, item.icon, children.length ? children : undefined)
    })
